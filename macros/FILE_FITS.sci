...
function [x,y,typ]=FILE_FITS(job,arg1,arg2)
// Copyright INRIA
x=[];y=[];typ=[]
select job
case 'plot' then
...
case 'set' then
  ...
  //dstate=model.in
  while %t do
    [ok,ftype,fname,heritance,exprs]=getvalue(..
	'Set FITS file parameters',..
	   ['fits file type (0-10)'; 
	   'Output file name';
    ...
    if ok then
      ipar=[ftype;length(fname);str2code(fname)]
      model.dstate=[];
      model.ipar=ipar
...
case 'define' then
  fname='outfile'
  model=scicos_model()
  model.sim=list('file_fits',4)
  model.in=-1
  model.out=-1
  model.evtin=1
  model.ipar=[0;length(fname);str2code(fname)]
  model.blocktype='c'
  model.dep_ut=[%t %f]
  
  exprs=[string(ftype); fname; string(0)'];
  gr_i=['txt=['' fits ''];';
