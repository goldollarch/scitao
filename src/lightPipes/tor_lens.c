...
FIELD lp_tor_lens( FIELD field, double R, double f, double x_shift, double y_shift )
{

    if (field.double1 != 0.) {
		printf( "tor_ens can not be applied in spherical\
				coordinates,\nuse CONVERT first\n");
        exit(1);
    }
   
    t_lens( R, -f, x_shift, y_shift ,field); 
